...
public class SearchPanelView extends FrameLayout implements
       ...
       final Runnable SetLongPress = new Runnable () {
            public void run() {
                if (!mSearchPanelLock) {
                    ...
                    maybeSkipKeyguard();
                    NavbarAction.getInstance(mContext).launchAction(longList.get(mTarget));
        ...
        public void onTrigger(View v, final int target) {
            mTarget = target;
            if (!mLongPress) {
                if (NavbarAction.ACTION_ASSIST.equals(intentList.get(target))) {
                    startAssistActivity();
                } else {
                    maybeSkipKeyguard();
                    NavbarAction.getInstance(mContext).launchAction(intentList.get(target));
                }
                mHandler.removeCallbacks(SetLongPress);
            }
        }
    ...
    private void setDrawables() {
        ...
        int middleBlanks = 0;
         ...
         if (screenLayout() == Configuration.SCREENLAYOUT_SIZE_LARGE || screenLayout() ==
                    Configuration.SCREENLAYOUT_SIZE_XLARGE || isScreenPortrait()) {
             startPosOffset =  1;
