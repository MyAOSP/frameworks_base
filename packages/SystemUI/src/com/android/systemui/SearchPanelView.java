...
public class SearchPanelView extends FrameLayout implements
       ...
       final Runnable SetLongPress = new Runnable () {
            public void run() {
                if (!mSearchPanelLock) {
                    ...
                    maybeSkipKeyguard();
                    Action.launchAction(mContext, longList.get(mTarget));
        ...
        public void onTargetChange(View v, final int target) {
            if (target == -1) {
                ...
                if (mBoolLongPress && !TextUtils.isEmpty(longList.get(target)) &&
                        !longList.get(target).equals(ActionConstant.ACTION_NULL.value())) {
                    mTarget = target;
        ...
        public void onTrigger(View v, final int target) {
            mTarget = target;
            if (!mLongPress) {
                if (ActionConstant.ACTION_ASSIST.equals(intentList.get(target))) {
                    startAssistActivity();
                } else {
                    maybeSkipKeyguard();
                    Action.launchAction(mContext, intentList.get(target));
                }
                mHandler.removeCallbacks(SetLongPress);
            }
        }
    ...
    private void setDrawables() {
        mLongPress = false;
        mSearchPanelLock = false;
        String tgtCenter = Settings.System.getString(mContext.getContentResolver(),
        ...
        for (int i = 0; i < startPosOffset; i++) {
            ...
            intentList.add(ActionConstant.ACTION_NULL.value());
            longList.add(ActionConstant.ACTION_NULL.value());
        }
        // Add User Targets
    ...
    private boolean hasValidTargets() {
        for (String target : targetActivities) {
            if (!TextUtils.isEmpty(target) && !target.equals(ActionConstant.ACTION_NULL.value())) {
                return true;
            }
        }
        return false;
...
//            });
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            for (int i = 0; i < 5; i++) {
                resolver.registerContentObserver(Settings.System.getUriFor(
                        Settings.System.SYSTEMUI_NAVRING[i]), false, this);
                resolver.registerContentObserver(Settings.System.getUriFor(
                        Settings.System.SYSTEMUI_NAVRING_LONG[i]), false, this);
                resolver.registerContentObserver(Settings.System.getUriFor(
