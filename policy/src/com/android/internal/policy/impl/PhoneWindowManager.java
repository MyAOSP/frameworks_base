...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void setInitialDisplaySize(Display display, int width, int height) {
        ...
        boolean mForceTabletUI = Settings.System.getInt(mContext.getContentResolver(),
            Settings.System.FORCE_TABLET_UI, 0) == 1;

        if (shortSizeDp < 600 && !mForceTabletUI) {
            // 0-599dp: "phone" UI with a separate status & navigation bar
            mHasSystemNavBar = false;
            mNavigationBarCanMove = true;
            Settings.System.putInt(mContext.getContentResolver(),
                    Settings.System.TABLET_UI, 0);
        } else if (shortSizeDp < 720 && !mForceTabletUI) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        boolean isWakeKey = (policyFlags
            ...
            if (isWakeKey && (!mVolumeWakeScreen || isOffByProx)
                        && ((keyCode == KeyEvent.KEYCODE_VOLUME_UP) || (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN))) {
                isWakeKey = false;
            }

            // music is playing, don't wake the screen in case we need to skip track
            if (isMusicActive() && mVolBtnMusicControls && mVolumeWakeScreen && isWakeKey
                        && ((keyCode == KeyEvent.KEYCODE_VOLUME_UP) || (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN)))
                isWakeKey = false;
